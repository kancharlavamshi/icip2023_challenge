...
class Inference(nn.Module):
    ...
    def run(self, low_img_path):
      ld=os.listdir(low_img_path)
      for image_input in ld:
        print('img path',image_input)
        file_name = os.path.basename(image_input)
        print(file_name)
        name = file_name.split('.')[0]
        low_img = self.transform(Image.open(low_img_path+'/'+image_input)).unsqueeze(0)
        enhance, p_time = self.forward(input_low_img=low_img)
        if not os.path.exists(self.opts.output):
            os.makedirs(self.opts.output)
        save_path = os.path.join(self.opts.output, file_name)
...
if __name__ == "__main__":
    ...
    # model path
    parser.add_argument('--Decom_model_low_path', type=str, default="/content/URetinex-Net/ckpt/init_low.pth")
    parser.add_argument('--unfolding_model_path', type=str, default="/content/URetinex-Net/ckpt/unfolding.pth")
    parser.add_argument('--adjust_model_path', type=str, default="/content/URetinex-Net/ckpt/L_adjust.pth")
