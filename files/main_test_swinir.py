...
def main():
    ...
    parser.add_argument('--Output_path', type=str, default=None, help='path to save the output images')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        path1 = path.removesuffix('.jpg')
        path_in = args.folder_lq
        path_out = args.Output_path
        #print(path_in)
        path1=path1.removeprefix(str(path_in))  
        print('idx',idx,'path',path1)
        ...
        height_V, width_V, channels_v = img_v.shape
        bigger = cv2.resize(output, (width_V, height_V))
        print('height',height_V)
        cv2.imwrite(str(path_out)+path1+'.jpg', bigger)
